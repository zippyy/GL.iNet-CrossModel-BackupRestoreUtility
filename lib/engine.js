...
function buildRemoteCommand({ opId, action, args, level = 'info', deferReload = false, trace = false }) {
...
export async function streamRuntime({ client, opId, action, args, level = 'info', deferReload = false, trace = false, timeout = 300000, onStdout = null }) {
  const bundle = await runtimeBundle();
  const command = buildRemoteCommand({ opId, action, args, level, deferReload, trace });
...
export async function remoteFacts({ client, opId, level }) {
  const result = await streamRuntime({ client, opId, action: 'facts', args: [], level, timeout: 60000 });
...
export async function remoteCreate({
  ...
  scripts = [], binaries = [], outputFile, level = 'info', timeout = 600000
}) {
