...
export function authConfigFromEnv(env = process.env) {
  ...
  if (plain) {
    ...
    return { verify: (pw) => crypto.timingSafeEqual(Buffer.from(String(pw)), Buffer.from(String(plain))) };
  }
  throw new Error(
    'No admin credential configured. Set ADMIN_PASSWORD_HASH (scrypt salt:hash) or ADMIN_PASSWORD ' +
    '(min 8 chars) via environment or Docker secret. The server refuses to start unauthenticated.'
  );
}
