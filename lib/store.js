...
export class Store {
  ...
  async prune() {
    ...
    // Never prune a profile a queued/running job still needs (e.g. a restore
    // about to read it) — mirrors the native rule that an active restore's
    // rollback snapshot must never be deleted during pruning.
    const activeProfileIds = new Set(
      (await this.listJobs())
        .filter((j) => ['running', 'queued'].includes(j.state) && j.profileId)
        .map((j) => j.profileId)
    );
    let total = 0;
    const candidates = [];
    for (const item of list) {
      if (activeProfileIds.has(item.id)) continue;
      total += item.size;
  ...
  async saveJob(job) {
    // Secrets never reach disk: strip password/privateKey and any nested
    // secret material before the atomic write.
    const { password, privateKey, _secrets, payload, ...safe } = job;
    const safePayload = { ...(payload || {}) };
    if (safePayload._secrets) delete safePayload._secrets;
    if (safePayload.connection?.password) safePayload.connection = { ...safePayload.connection, password: undefined, privateKey: undefined };
    await atomicWrite(this.jobPath(job.id), JSON.stringify({ ...safe, payload: safePayload }, null, 2));
    return { ...safe, payload: safePayload };
  }
