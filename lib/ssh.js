...
export async function connectRouter(input, { logger, trustHostKey, hostKeyProvider } = {}) {
  const connection = normalizeConnection(input, { hostKeyProvider });
  ...
  // so an accepted new host key can be persisted. The option is authoritative:
  // normalizeConnection intentionally drops unknown fields, so the wrapper
  // must forward the trust policy explicitly (regression: accept-new used to
  // connect without ever persisting the fingerprint when reached through
  // withRouter).
  connection.trustHostKey = trustHostKey || connection.trustHostKey || null;
...
export async function withRouter(connectionInput, task, { hostKeyProvider, trustHostKey, logger } = {}) {
  // Normalize once here so validation failures surface before a connection is
  // attempted, then forward BOTH the host-key provider and the trust policy to
  // connectRouter as explicit options. connectRouter re-normalizes its input,
  // so options are the only channel guaranteed to survive; without this the
  // accept-new trust callback was dropped and first-connection fingerprints
  // were never persisted through the wrapper.
  const connection = normalizeConnection(connectionInput, { hostKeyProvider });
  const { client, connection: resolved } = await connectRouter(connection, { logger, trustHostKey });
  try {
