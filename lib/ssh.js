...
export function normalizeConnection(input = {}, { hostKeyProvider } = {}) {
  ...
  const connection = { host, port, username, auth, hostKeyProvider: hostKeyProvider || (() => Promise.resolve(null)) };
...
export async function connectRouter(input, { logger } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    client.on('error', (error) => {
      ...
      if (text.includes('hostkey') || text.includes('host key')) {
